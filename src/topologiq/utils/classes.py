...
class SingleBeam:
    ...
    def intersects_co_planarity(self, other: 'SingleBeam') -> bool:
        ...
        # The dot product will describe which case of directions we're dealing with; same, opposite, orthogonal
        relative = SingleBeam.__dot_product(d1, d2)
        # sigma is the position of the source of the other beam relative to the source of this beam
        sigma = SingleBeam.__vector_subtraction(p2, p1)

        if relative == 0:
            # The beams are in orthogonal directions
            basis = SingleBeam.__vector_subtraction(d1, d2)
            intra = True
            pos_q = True
            for i in range(3):
                # Check that the source of the other beam lies in the subspace generated by the directions
                intra = intra and (basis[i] != 0 or sigma[i] == 0)
                # Check that the source of the other beam lies in the positive quadrant of said subspace
                pos_q = pos_q and (sigma[i] * basis[i] >= 0)
            intersecting_rays = intra and pos_q
        else:
            # The beams are in the same or opposite directions
            # Either way, the source must be located along the same axis as the beam direction of this beam
            cross = SingleBeam.__cross_product(d1, sigma)
            intersecting_rays = SingleBeam.__dot_product(cross, cross) == 0
            if relative == -1:
                intersecting_rays = intersecting_rays and SingleBeam.__dot_product(d1, sigma) >= 0
