...
class SingleBeam:
    ...
    def intersects(self, other: 'SingleBeam', len_of_materialised_beam: int) -> bool:
        ...
        if SingleBeam.INTERSECTION_CONSISTENCY_CHECKS:
            ...
            # intersecting_arrays = any([self.contains(c) for c in other_as_array])
    ...
    def intersects_co_planarity(self, other: 'SingleBeam') -> bool:
        ...
        orientation = SingleBeam.__dot_product(d1, d2)
        ...
        if orientation == 0:
            ...
            # of the subspace generated by {d1, -d2}
            basis = SingleBeam.__vector_subtraction(d1, d2)
            intersecting_rays = True
            for i in range(3):
                intersecting_rays &= sigma[i] == 0 or SingleBeam.__sign(sigma[i]) == SingleBeam.__sign(basis[i])
        else:
            ...
            if orientation == -1:
                ...
                intersecting_rays &= SingleBeam.__dot_product(d1, sigma) >= 0
